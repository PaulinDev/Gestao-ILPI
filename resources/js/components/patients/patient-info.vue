<template>
  <div>
    <v-card class="mx-auto my-12 pt-10">
        ...
        >
          <template v-slot:activator="{ on, attrs }">
            <v-btn color="primary" class="mb-2" v-bind="attrs" v-on="on" text>
              {{ patientInfo.photo === null ? "Nova foto" : "Alterar foto" }}
            </v-btn>
          </template>
          <v-card>
            <v-card-text>
      ...
      <v-card-actions v-if="!relative">
        <v-dialog v-model="dialogEditPatient" persistent max-width="auto">
          <template v-slot:activator="{ on, attrs }">
            <v-btn color="warning" v-bind="attrs" v-on="on" text>
              Editar
            </v-btn>
          </template>
          <edit-patient
        ...
        <v-btn color="error" v-else text @click="disableOrActivePatient(false)">
          Desativar
        </v-btn>
      </v-card-actions>
    </v-card>
...
export default {
  ...
  methods: {
    ...
    disableOrActivePatient(active) {
      ...
      axios
        .put(this.urlBaseApi + "/" + this.idCurrentPatient, data, settings)
        .then(() => {
